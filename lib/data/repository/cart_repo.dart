...
class CartRepo{
  ...
  Future<ApiResponse> GetBuisnessUser(email) async {
    try {
      final response = await dioClient.get(AppConstants.GET_BUISNESS_USERS+'?name='+email);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  }
  Future<ApiResponse> GetLimitamount(String userId) async {
    try {
      final response = await dioClient.get(AppConstants.GET_LIMIT_AMOUNT+userId);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
