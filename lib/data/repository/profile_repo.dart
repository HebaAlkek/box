...
class ProfileRepo {
  ...
  Future<ApiResponse> refreshData(String code) async {
    try {
      final response = await dioClient.get(AppConstants.REFRESH_DATA+code+'&key=123');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  }
  Future<ApiResponse> getUserInfoBuisness() async {
    try {
      final response = await dioClient.get(AppConstants.CUSTOMER_URI_buisness);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> addDataBala(     Map<String, dynamic> data) async {
    try {
      Response response = await dioClient.post(
        AppConstants.ADD_BALA,
        data: data,
      );
      return ApiResponse.withSuccess(response);
    } catch (e) {
  ...
  Future<http.StreamedResponse> updateProfileComm(UserInfoModel userInfoModel, String pass, File file, String token,
      String comname,String comnum,String vatnum,File comphoto,File vatphoto,File natphoto) async {
    http.MultipartRequest request = http.MultipartRequest('POST', Uri.parse('${AppConstants.BASE_URL}${AppConstants.UPDATE_PROFILE_URI}'));
    request.headers.addAll(<String,String>{'Authorization': 'Bearer $token'});
    if(file != null){
      request.files.add(http.MultipartFile('image', file.readAsBytes().asStream(), file.lengthSync(), filename: file.path.split('/').last));
    ...
    if(comphoto != null){
      request.files.add(http.MultipartFile('commerical_photo', comphoto.readAsBytes().asStream(), comphoto.lengthSync(), filename: comphoto.path.split('/').last));
    }
    if(vatphoto != null){
      request.files.add(http.MultipartFile('vat_photo', vatphoto.readAsBytes().asStream(), vatphoto.lengthSync(), filename: vatphoto.path.split('/').last));
    }
    if(natphoto != null){
      request.files.add(http.MultipartFile('natinal_photo', natphoto.readAsBytes().asStream(), natphoto.lengthSync(), filename: natphoto.path.split('/').last));
    }
    Map<String, String> _fields = Map();
    if(pass.isEmpty) {
      _fields.addAll(<String, String>{
        '_method': 'put', 'f_name': userInfoModel.fName, 'l_name': userInfoModel.lName, 'phone': userInfoModel.phone,'vat_number':vatnum,'comerical_number':comnum,'company_name':comname
      });
    }else {
      _fields.addAll(<String, String>{
        '_method': 'put', 'f_name': userInfoModel.fName, 'l_name': userInfoModel.lName, 'phone': userInfoModel.phone, 'password': pass,'vat_number':vatnum,'comerical_number':comnum,'company_name':comname
      });
    }
    request.fields.addAll(_fields);
    print('========>${_fields.toString()}');
