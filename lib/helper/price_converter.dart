...
class PriceConverter {
  static String convertPrice(BuildContext context, double price, {double discount, String discountType}) {
    ...
    return '${_inRight ? '' : ' ' +Provider.of<SplashProvider>(context, listen: false).myCurrency.symbol}'
        ...
        price * Provider.of<SplashProvider>(context, listen: false).myCurrency.exchangeRate
            * (1/Provider.of<SplashProvider>(context, listen: false).usdCurrency.exchangeRate)
    ).toStringAsFixed(Provider.of<SplashProvider>(context,listen: false).configModel.decimalPointSetting??1).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},'))}'
