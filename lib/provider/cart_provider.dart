...
import 'package:flutter_sixvalley_ecommerce/buisness_user_model.dart';
...
class CartProvider extends ChangeNotifier {
  ...
  List<BuisnessUserModel> _buinessUsers = [];
 double _amountLimit  = 0.0;
  ...
  bool _getData = true;
  BuisnessUserModel buinessItem;
  List<BuisnessUserModel> get buinessUsers => _buinessUsers;
  ...
  double get amountLimit => _amountLimit;
  ...
  Future<List> GetBuisnessUser(BuildContext context,String email) async {
    _isLoadingB = true;
    List _list = [];

    // notifyListeners();
    ApiResponse apiResponse = await cartRepo.GetBuisnessUser(email);
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      _buinessUsers = [];
      apiResponse.response.data.forEach((cart) => _buinessUsers.add(BuisnessUserModel.fromJson(cart)));
      _list.addAll(_buinessUsers);
  ...
  Future<void> GetLimitAmount(BuildContext context,String userId) async {

    // notifyListeners();
    ApiResponse apiResponse = await cartRepo.GetLimitamount(userId);
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      _amountLimit = double.parse(apiResponse.response.data.toString());
