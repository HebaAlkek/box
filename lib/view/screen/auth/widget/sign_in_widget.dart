  ...
  route(bool isRoute, String token, String temporaryToken, String errorMessage) async {
    if (isRoute) {
      if(token==null || token.isEmpty){
        ...
        await Provider.of<ProfileProvider>(context, listen: false).getUserInfo(context).then((value) async => {
          if(value=='-1'){
            await Provider.of<ProfileProvider>(context, listen: false).getUserInfoBusiness(context,'1')
      ...
      child: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            widget.home=='1'?Visibility(child: Text(''),visible: false,):        Center(child: Text(getTranslated('OR', context),
                style: titilliumRegular.copyWith(fontSize: Dimensions.FONT_SIZE_DEFAULT))),

            //for order as guest
    widget.home=='1'?Visibility(child: Text(''),visible: false,):       GestureDetector(
              onTap: () {
                Provider.of<BrandProvider>(context, listen: false).brandList.clear();
              ...
              child: Container(
                ...
                child: Text(getTranslated('CONTINUE_AS_GUEST', context),
                    style: titleHeader.copyWith(color: Color(int.parse('0xFF' +
                        Provider.of<SplashProvider>(context, listen: false)
                        .configModel
                        .font_color
                        .split('#')[1])),)),
